...
function buildInvoiceHTML(exportMode = 'html') {
  ...
  const logoSrc = window.LOGO_BASE64 || "logo.png";
  const headerHTML = `
  ...
  window.onload = () => {
    setTimeout(() => {
      ...
      const opt = {
        ...
        html2canvas:  { scale: 2, height: element.offsetHeight - 1 },
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .page{
    width:210mm;
    min-height:296.5mm; /* Changed to 296.5mm to prevent bottom white gap */
    background:#fff;
    padding:0;
    display:flex;
...
function downloadPDF() {
  ...
  document.querySelectorAll('.btn-download-pdf').forEach(btn => {
    ...
    setTimeout(() => { btn.textContent = '⬇ PDF'; btn.disabled = false; }, 3000);
  });
